...
public class SetCommand {
    ...
    public static int setCommand(CommandContext<CommandSourceStack> ctx, Collection<ServerPlayer> players, int amount) {
        ...
        ctx.getSource().sendSuccess(() -> Component.literal("Updated balance of " + players.size() + " players to " + DiamondEconomyConfig.formatCurrency(amount)), true);
        return players.size();
    }
    ...
    public static int setCommand(CommandContext<CommandSourceStack> ctx, int amount, boolean shouldModifyAll) throws CommandSyntaxException {
        if (shouldModifyAll) {
            ...
            ctx.getSource().sendSuccess(() -> Component.literal("Updated all accounts balance to " + DiamondEconomyConfig.formatCurrency(amount)), true);
        } else {
            DiamondUtils.getDatabaseManager().setBalance(ctx.getSource().getPlayerOrException().getStringUUID(), amount);
            ctx.getSource().sendSuccess(() -> Component.literal("Updated your balance to " + DiamondEconomyConfig.formatCurrency(amount)), true);
        }
        return 1;
    }
}
